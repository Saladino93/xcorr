...
def get_completeness_map(nside: int, fracgood: np.ndarray, healpix_positions: np.ndarray) -> np.ndarray:
    shape = hp.nside2npix(nside)
    completeness_map = np.ones(shape)*hp.pixelfunc.UNSEEN
...
def get_average_number_density_gg_per_arcmin2(counts: np.ndarray, mask: np.ndarray, completeness_map: np.ndarray, pix_area_out_arcmin2: float):
    ...
    average_number_density_gg = np.nansum(np.nan_to_num(counts/completeness_map))/(pix_area_out_arcmin2*np.sum(mask))
